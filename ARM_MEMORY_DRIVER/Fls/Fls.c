...
#define FLASH	((volatile Fls_Reg*)0x40023C00)
...
void Fls_vidInit(const Fls_Config* ConfigPtr) {
	u8 Loc_u8TimerOut = 0;
...
}/*end of function Fls_vidInit*/

Std_ReturnType Fls_enuMassErase(void) {
	u8 Loc_u8TimerOut = 0;
	Std_ReturnType Loc_enuErrorStatus = Fpro_enuOK;
	...
	if (Loc_u8TimerOut == 20) {
		/*The register is still lock*/
		Loc_enuErrorStatus = Fls_enuControlRegisterLock;
...
Std_ReturnType Fls_enuSectorErase(u8 SectorNumber) {

	u32 Loc_u32TempRegister;
	u8 Loc_u8TimerOut = 0;
	Std_ReturnType Loc_enuErrorStatus = Fpro_enuOK;
	if (SectorNumber > MAX_SECTOR_NUMBER) {
		Loc_enuErrorStatus = Fls_enuSectorNumberWrong;
	}
	else
	{
		while ((FLASH->SR >> SR_BUSY_BIT) == 1) {
...
Std_ReturnType Fls_enuOneTimeProgramming(pu32 Data, u8 BufferSize) {
...
Std_ReturnType Fls_Write(Fls_AddressType TargetAddress, Fls_LengthType Length, u64 Data) {

	u8 Loc_u8TimerOut = 0;
	u32 Loc_u32TempRegister = 0;
	Std_ReturnType Loc_enuErrorStatus = Fpro_enuOK;

	if (TargetAddress > 0x0803FFFF || TargetAddress < 0x08000000) {
		Loc_enuErrorStatus = Fls_enuExceedFlash;
	}
	else
	{
		...
		if (Loc_u8TimerOut == 20) {
			...
			Loc_u32TempRegister |= 1 << CR_PG | Length << CR_PSIZE_BIT;
			...
			case PAGESIZE_BYTE:
				(*(volatile u32*)TargetAddress) = (u8)Data;
				break;

			case PAGESIZE_HALF_WORD:
				(*(volatile u32*)TargetAddress) = (u16)Data;
				break;

			case PAGESIZE_WORD:
				(*(volatile u32*)TargetAddress) = (u32)Data;
				break;

			case PAGESIZE_DOUBLE_WORD:
				(*(volatile u32*)TargetAddress) = (u64)Data;
				break;
